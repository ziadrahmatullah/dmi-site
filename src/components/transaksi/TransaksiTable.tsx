...
import type { Transaction, Meta } from "@/types/transaksi";
...
function Pagination({ meta, onPageChange }: { meta: Meta; onPageChange: (p: number) => void }) {
  ...
  const btnBase =
    "h-8 min-w-[32px] px-2 rounded-lg text-sm font-medium border transition-all duration-150 cursor-pointer";
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
      <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
        <h3 className="font-bold text-gray-900 text-sm">Daftar Transaksi</h3>
        {!loading && (
          <span className="text-xs text-gray-400">{meta.total_item} transaksi</span>
        )}
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              {["Tanggal", "Deskripsi", "Kategori", "Lokasi", "Jumlah", "Aksi"].map((h) => (
                <th
                  key={h}
              ...
              <tr>
                <td colSpan={6} className="py-16 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="font-medium text-gray-600 text-sm">Belum ada data transaksi</p>
              ...
              transactions.map((tx) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-gray-800 max-w-[200px]">
                    <span className="line-clamp-2 leading-snug">{tx.description}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        tx.category_type === "pemasukan"
                          ? "bg-green-100 text-green-700"
                          : "bg-rose-100 text-rose-700"
                      }`}
                    >
                      {tx.category?.name ?? tx.category_type}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-gray-600 text-xs whitespace-nowrap">
                    <span className="text-gray-800 font-medium">{tx.district?.name}</span>
                    {tx.mosque?.name && (
                      <>
                        <span className="text-gray-400 mx-1">•</span>
                        <span>{tx.mosque.name}</span>
                      </>
                    )}
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span
                      className={`font-bold text-sm ${
                        tx.category_type === "pemasukan" ? "text-green-600" : "text-rose-600"
                      }`}
                    >
                      {tx.category_type === "pengeluaran" ? "−" : "+"}
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <button
                        onClick={() => console.log("view", tx.id)}
